...
class GeneMergeValidationOutput < ValidationReport
  ...
  def initialize (slope, unimodality, threshold_down = 0.4, threshold_up = 1.2, expected = :no)
    ...
    @expected       = expected
    @plot_files     = []
    @approach       = 'We expect the query sequence to encode a single protein-coding' 
                      ' gene. Here, we' \
                      ' analyse the High-scoring Segment Pairs (HSPs)' \
                      ' identified by BLAST to determine whether the query '\
                      ' includes sequence from two or more genes.'
    @explanation    = explain
    @conclusion     = conclude
  end
