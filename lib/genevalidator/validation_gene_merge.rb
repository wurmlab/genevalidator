...
class GeneMergeValidation < ValidationTest
  ...
  def plot_matched_regions(output = "#{filename}_match.json", hits = @hits, prediction = @prediction)
      ...
      if ratio == 0
        f.write((hits.each_with_index.map{|hit, i| {"y"=>i, "start"=>0, "stop"=>prediction.xml_length, "color"=>"gray"}} +
        ...
        f.write((hits.select.each_with_index {|hit, i| i%ratio==0}.each_with_index.map{|hit, i| {"y"=>i, "start"=>0, "stop"=>prediction.xml_length, "color"=>"gray"}} +
