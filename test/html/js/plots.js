...
function addPlot(target, filename, type, title, footer, xtitle, ytitle, aux1, aux2){
	...
	switch(type){
		...
		case "bars":
		  ...
		  break;
		case "lines":
		  plot_lines(filename, target, title, legend, xtitle, ytitle)
		  break;
		default:
		  break;
	}	
...
function plot_lines(filename, target, title, footer, xTitle, yTitle){
	...
	d3.json(filename, function(error, data) {
          ...
          var ratio = Math.floor(data.length/no_lines)
	  
	  if (ratio != 0)
	     data = data.filter(function(d,i) { return i%(ratio)==0; });
	  ...
	  svg.selectAll(".dot")
		...
		.enter().append("line")
				  ...
				  .attr("stroke-width", height/data.length)
